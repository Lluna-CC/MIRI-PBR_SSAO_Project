...
class GLWidget : public QOpenGLWidget, protected QOpenGLFunctions_3_3_Core {
 ...
 public:
  ...
  void GenerateSpecularIrradiance();
 protected:
  /**
 ...
 private:
  ...
  void renderSecondStep();

  void renderSSAO();
  
  void renderBlur();
  void paintDiffuseIrradiance(int face);

  void InitializeSSAOBuffers();
  void generatePrefilteredMaps();
  /**
  ...
  std::vector<std::unique_ptr<QOpenGLShaderProgram>> secondStepPrograms_;

  QOpenGLShaderProgram diffuse_generator;
  QOpenGLShaderProgram specular_generator;
  ...
  GLuint imrpovedAO_FBO;
  GLuint imrpovedAO_Text;

  GLuint noiseText;
  
  bool generate_diffuse = false;
  bool generate_prefilter = false;
  bool twoStep = false;
  int twoStepMode = 0;
  bool AOset = false;
  bool AOAlbedo = false;
  bool imrpovedAO = false;
  ...
  /**
   * @brief SetTwoStep enables the two step renderer
   */
  void SetTwoStep(bool set);


  /**
   * @brief SetTwoStepMode selects the mode of the Two Step renderer
   */
   void SetTwoStepMode(int i);

   /**
   * @brief set AO activates the AO estimation
   */
   void SetAO(bool set);

     /**
   * @brief sets the radius of the AO kernel
   */
   void SetRadius(double r);

     /**
   * @brief sets the number of directions of the AO kernel
   */
   void SetDirections(int dir);

      /**
   * @brief sets the number of samples of the AO kernel
   */
   void SetSamples(int sampl);

   
  /**
   * @brief sets the visualization of the albedo in the SSAO as oposed to just the geometry
  */
   void SetAOAlbedo(bool set);

   /**
   * @brief sets the improved SSAO shader
  */
  void SetImprovedSSAO(bool set);

   /**
   * @brief sets the usual PBR and deactivates all AO and two step methods
  */
   void SetPBR(bool set);
